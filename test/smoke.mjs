...
try {
  ...
  const enriched=await(await expectOk("/api/research-dashboard?memberId=member-jin")).json();const enrichedMember=enriched.members[0];const enrichedSite=enrichedMember.sites.find(item=>item.companyId==="co-xinyuan-logistics");if(enrichedSite.evidenceCount!==1||enrichedSite.confirmedProblemCount!==1||enrichedSite.solutionCount!==1||enrichedSite.validatedQuestionCount!==3||enrichedSite.confirmedQuestionCount!==1||enrichedSite.partialQuestionCount!==1||enrichedSite.refutedQuestionCount!==1||enrichedSite.questionValidationPercent!==17||enrichedSite.closurePercent!==79)throw new Error("dynamic dashboard counts mismatch");if(enrichedMember.sitesComplete!==enrichedMember.sites.filter(site=>site.questionsComplete).length||enrichedMember.totalSites!==enrichedMember.sites.length)throw new Error("member site summary is stale");

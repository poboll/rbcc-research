...
try {
  ...
  const generatedQuestions = Array.from({ length: 18 }, (_, index) => ({ text: `冒烟问题 ${index + 1}`, lens: "pending", tags: ["验证"], ...(index===0?{validationOutcome:"confirmed",evidenceSource:"smoke-evidence-1",answer:"现场记录支持该判断"}:{}) }));
  ...
  const problem=await(await expectOk("/api/evidence/synthesize",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"problem",memberId:"member-jin",memberName:"金俊烯",companyId:"co-xinyuan-logistics",companyName:"信源物流",observations:observations.observations})})).json();if(problem.questionIds?.length!==1)throw new Error("validated question to problem linkage mismatch");await expectOk(`/api/problems/${problem.id}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({validationOutcome:"confirmed",validationNote:"现场留痕与逐题验证交叉支持"})});

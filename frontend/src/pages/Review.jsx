...
export function ReviewHome() {
  ...
  return <div className="review-page page-pad">
    ...
    <section className="representative-sites"><header><h2>队员代表站点 · 深链</h2><p>快速进入每位队员的一份万字评审报告</p></header>{loading ? <div className="review-empty"><Loading label="正在汇总队员报告…" /></div> : representative.length ? <div>{representative.map(({group,report})=><article key={group.memberId}><span><strong>{group.memberName}</strong><small>{report.placeName} · 第{report.day||1}天</small></span><AppLink href={`/dashboard?memberId=${group.memberId}&companyId=${report.companyId}`}>编辑台</AppLink><AppLink href={`/review/report?memberId=${group.memberId}&companyId=${report.companyId}`}>万字</AppLink></article>)}</div> : <div className="review-empty"><FileCheck2 size={22}/><span><strong>暂未汇总可评审报告</strong><small>队员完成站点报告后，会自动出现在这里。</small></span><AppLink href="/dashboard">前往报告编辑台</AppLink></div>}</section>
    {members.length ? <ReportRows /> : null}
  </div>;
}

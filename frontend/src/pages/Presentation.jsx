...
const SCENES = [
  ...
  ["章节","第六章 · 红小八","最后看知识如何反哺现场。","chapter"],
  ["红小八","检索组内知识","优先检索挂载资料、站点情报和团队纪要。","agent"],
  ["红小八","协作对话与提醒","回答标注来源，支持通知和语音播报。","agent"],
...
export function Roadshow() {
  ...
  return <div className="roadshow page-pad">
    ...
    <section className={`roadshow-stage scene-${current[3]}`} key={scene}><div className="stage-number">{String(scene+1).padStart(2,"0")}</div><div><small>{current[0]}</small><h2>{current[1]}</h2><p>{current[2]}</p>{sceneLinks[current[3]]?<AppLink className="scene-link" href={sceneLinks[current[3]]}>打开对应功能 <ArrowRight size={14}/></AppLink>:null}</div><div className="stage-metrics"><span><strong>{data?.summary?.siteAssignmentCount || 0}</strong>人次</span><span><strong>{data?.summary?.sitesQuestionsComplete || 0}</strong>问题完成</span><span><strong>{data?.summary?.uniqueSiteCount || 0}</strong>节点</span></div></section>

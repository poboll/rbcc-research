...
function RouteCapture({ member }) {
  ...
  async function loadSite() {
    ...
    setQuestions(questionData.questions ?? []); setProblems(problemData.problems ?? []); setSolutions(solutionData.solutions ?? []);
  }
  useEffect(() => { void loadSite(); }, [site?.companyId]);

  async function suggestQuestions() {
    if (!site) return; setBusy("suggest"); setStatus("");
    try { const result = await json("/api/research-report/suggest", jsonOptions("POST", { memberId:member.id, companyId:site.companyId })); setQuestions(result.questions ?? []); setStatus(`已生成 ${result.questions?.length ?? 0} 个可验证问题，请检查后保存。`); }
    catch (error) { setStatus(error.message); } finally { setBusy(""); }
  }
  async function saveQuestions() {
    if (!site) return; setBusy("questions");
    try { const result=await json("/api/research-questions",jsonOptions("PUT",{memberId:member.id,companyId:site.companyId,questions}));setQuestions(result.questions);setStatus("问题清单已保存并同步到报告工作台。"); }
    catch(error){setStatus(error.message)}finally{setBusy("")}
  }

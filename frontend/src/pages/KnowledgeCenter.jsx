...
export function KnowledgeCenterPage() {
  ...
  return <div className="knowledge-center page-pad">
    <header className="knowledge-heading"><div><BookOpen size={22}/><span><small>知识运营 · 证据入口</small><h1>调研知识中心</h1><p>挂载团队资料，建立可追溯检索片段，为红八宝与万字报告提供引用依据。</p></span></div><div className="knowledge-heading-actions"><AppLink href={`/agent${query.trim()?`?prompt=${encodeURIComponent(`请基于知识库分析：${query.trim()}`)}`:""}`}><Bot size={14}/>交给红八宝</AppLink><button onClick={() => void load()}><RefreshCw size={14}/>刷新索引</button></div></header>
    <section className="knowledge-flow" aria-label="知识进入调研产出的流程">{[["01",FileUp,"资料挂载","PDF / DOCX / 纪要"],["02",Database,"解析索引","来源与片段可追溯"],["03",Search,"检索验证","查企业、证据与痛点"],["04",Bot,"红八宝提炼","事实、推断、待验证"],["05",FileCheck2,"报告引用","进入四核报告"]].map(([index,Icon,title,text],step)=><React.Fragment key={index}><article><span>{index}</span><Icon size={16}/><div><strong>{title}</strong><small>{text}</small></div></article>{step<4?<ArrowRight size={14}/>:null}</React.Fragment>)}</section>
    ...
    {tab === "search" ? <><section className="knowledge-handoff"><div><Link2 size={16}/><span><strong>把检索结果继续用于调研</strong><small>红八宝会带着当前问题检索知识；报告编辑台用于整理引用后的正式产出。</small></span></div><AppLink href={`/agent?prompt=${encodeURIComponent(`请基于知识库分析“${query.trim()}”，区分事实、推断与待验证项，并给出下一步。`)}`}><Bot size={14}/>询问红八宝</AppLink><AppLink href="/dashboard"><FileCheck2 size={14}/>进入报告</AppLink></section><section className="knowledge-results">{results.map(item=><article key={`${item.sourceKind}-${item.id}`}><header><strong>{item.title}</strong><span>相关度 {item.score}</span></header><p>{item.content}</p><footer><small>{item.sourceKind==="mounted"?`来源片段 ${(item.position??0)+1}`:"知识条目"} · {(item.tags??[]).join(" · ")||"未分类"}</small><AppLink href={`/agent?prompt=${encodeURIComponent(`请结合“${item.title}”分析：${query.trim()}`)}`}>继续提问 <ArrowRight size={12}/></AppLink></footer></article>)}{!results.length?<Empty>没有命中。可换一个关键词，或先在“资料挂载”中添加团队资料。</Empty>:null}</section></> : null}

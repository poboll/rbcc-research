...
import { BookOpen, Database, FileSearch, FileText, FileUp, Plus, RefreshCw, Search, Trash2 } from "lucide-react";
...
export function KnowledgeCenterPage() {
  ...
  async function search(event) {
    ...
    setSearching(true);
    try { setResults((await json(`/api/knowledge/search?q=${encodeURIComponent(query)}&limit=24`)).results ?? []); setTab("search"); }
  ...
  return <div className="knowledge-center page-pad">
    <header className="knowledge-heading"><div><BookOpen size={22}/><span><small>KNOWLEDGE OPERATIONS</small><h1>调研知识中心</h1><p>挂载团队资料，建立可追溯检索片段，为红小八与万字报告提供引用依据。</p></span></div><button onClick={() => void load()}><RefreshCw size={14}/>刷新索引</button></header>
    ...
    {tab === "search" ? <section className="knowledge-results">{results.map(item=><article key={item.id}><header><strong>{item.title}</strong><span>相关度 {item.score}</span></header><p>{item.content}</p><small>来源片段 {item.position+1} · {(item.tags??[]).join(" · ")}</small></article>)}{!results.length?<Empty/>:null}</section> : null}

...
export function AdminPage() {
  ...
  return <div className="admin-page page-pad">
    ...
    <Tabs value={tab} onChange={setTab} items={[{value:"overview",label:"总览"},{value:"routes",label:"成员与路线"},{value:"evidence",label:"证据数据"},{value:"knowledge",label:"知识挂载"},{value:"reports",label:"DOCX 定稿"}]}/>
    ...
    {tab === "knowledge" ? <section className="admin-knowledge"><header><div><h2>知识库挂载状态</h2><p>{counts.knowledgeSources} 个来源 · {counts.knowledgeChunks} 个索引片段 · {counts.knowledge} 条知识</p></div><a href="/knowledge">打开知识中心</a></header>{state.data.knowledgeSources.map(source=><article key={source.id}><FileUp size={15}/><div><strong>{source.title}</strong><small>{source.originalName} · {source.charCount?.toLocaleString()} 字</small></div><span>{source.status}</span></article>)}{!state.data.knowledgeSources.length?<Empty/>:null}</section> : null}

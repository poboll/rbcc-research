...
function MemberPerspective({ profile, selected, onSelect }) {
  ...
  return <button className={`perspective-card ${selected?"selected":""}`} onClick={onSelect}>
    ...
    <footer><span>{profile.questions.length} 问题</span><span>{profile.validated.length} 验证</span><span>{profile.media.length} 证据</span><ArrowRight size={14}/></footer>
  </button>;
}
...
export function CollabHubPage(){
  ...
  function chooseMember(id){const next=profiles.find(item=>item.memberId===id);setMemberId(id);setCompanyId(next?.sites?.[0]?.companyId||"");}
  ...
  return <div className="collab-hub milestone-hub page-pad">
    ...
    <section className="thinking-section"><header><div><Sparkles size={17}/><span><h2>五人思考矩阵</h2><p>每个人不是“完成了多少”，而是“提出了什么、依据是什么、下一步问什么”。</p></span></div><strong>{profiles.filter(item=>item.updates.length).length}/{profiles.length} 已提交观点</strong></header><div className="perspective-grid">{profiles.map(item=><MemberPerspective key={item.memberId} profile={item} selected={item.memberId===profile.memberId} onSelect={()=>chooseMember(item.memberId)}/>)}</div></section>

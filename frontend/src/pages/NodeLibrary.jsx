...
export function NodeLibraryPage() {
  ...
  return <div className="node-library page-pad">
    ...
    {kind !== "schedule" ? <section className="node-sections">{[1,2,3].filter(value => day === "all" || Number(day) === value).map(value => { const dayNodes = filtered.filter(node => node.day === value); return dayNodes.length ? <section key={value}><header><CalendarDays size={16}/><div><h2>Day {value}</h2><p>{[...new Set(dayNodes.map(node => routeLabels[node.routeId]))].join(" · ")}</p></div></header><div className="node-grid">{dayNodes.map(node => <button className={selected === node.id ? "selected" : ""} onClick={() => setSelected(current => current === node.id ? "" : node.id)} key={node.id}><div><span className="node-status">{node.questionsComplete ? "问题已同步" : "待走访"}</span><small>{node.themeName || "企业调研"}</small></div><h3>{node.companyName}</h3><p>{node.description}</p><footer><span><Users size={12}/>{node.memberName}</span><span><MapPin size={12}/>{routeLabels[node.routeId]}</span></footer>{selected === node.id ? <aside><strong>过程资产</strong><span>预设问题 {node.questions?.length || 0} 条</span><span>验证完成 {node.questionsValidatedCount || 0} 条</span><a href={`/dashboard?memberId=${node.memberId}&companyId=${node.companyId}`}>打开调研报告</a></aside> : null}</button>)}</div></section> : null; })}</section> : null}
  </div>;
}

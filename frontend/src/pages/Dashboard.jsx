...
import { Bot, Download, FileText, Lightbulb, RefreshCw, Save, Sparkles } from "lucide-react";
...
export function DashboardPage({search=""}) {
  ...
  async function suggest(){setBusy("suggest");try{setSuggestion(await json("/api/research-report/suggest",jsonOptions("POST",{memberId:member.memberId,companyId:site.companyId})));}catch(e){setError(e.message)}finally{setBusy("")}}
  async function applySuggestedQuestions(){if(!suggestion)return;setBusy("apply");try{await json("/api/research-report/suggest",jsonOptions("POST",{action:"applyQuestions",memberId:member.memberId,companyId:site.companyId,questions:suggestion.questions}));setSuggestion(current=>({...current,applied:true}));}catch(e){setError(e.message)}finally{setBusy("")}}
  return <div className="dashboard-page page-pad"><header className="page-heading"><div><FileText size={20}/><span><small>调研报告</small><h1>四核报告工作台</h1></span></div><div className="dashboard-actions"><button onClick={()=>void suggest()} disabled={!!busy}><Lightbulb size={14}/>{busy==="suggest"?"分析中…":"生成调研建议"}</button><button onClick={()=>void save()} disabled={!!busy}><Save size={14}/>{busy==="save"?"保存中…":"保存修正"}</button><button className="primary-button" onClick={()=>void generate()} disabled={!!busy}><Sparkles size={14}/>{busy==="generate"?"AI 分段生成中…":"生成万字报告"}</button><button onClick={()=>void download()} disabled={!!busy}><Download size={14}/>导出 Word</button></div></header>
    ...
    {error?<ErrorState message={error}/>:null}{busy==="load"?<Loading label="加载站点报告…"/>:null}{suggestion?<section className="report-suggestion"><header><div><Lightbulb size={18}/><span><h2>{suggestion.project?.title||"调研问题与试点建议"}</h2><p>{suggestion.project?.summary}</p></span></div><button className="primary-button" disabled={suggestion.applied||busy==="apply"} onClick={()=>void applySuggestedQuestions()}>{suggestion.applied?"已加入问题清单":"将 18 问加入队员端"}</button></header><div>{(suggestion.questions??[]).slice(0,6).map((item,index)=><span key={item.id||index}><strong>{index+1}</strong>{item.text}</span>)}</div></section>:null}
